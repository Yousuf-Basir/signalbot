...
5. Use the returned `transcript` for summarization, reply drafting, or project-context comparison.
6. When the user adds a project folder, inspect relevant project files normally, then combine that context with the Signal transcript. Keep the reply grounded in both sources.
...
Call `signalbot_get_messages` with the likely project group and `sender_name: "Yann"`, then inspect the project folder files the user provides. Draft a concise reply that answers Yann's ask and cites project facts when useful.
